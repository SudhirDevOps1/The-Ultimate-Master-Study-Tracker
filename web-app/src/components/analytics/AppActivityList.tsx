...
export function AppActivityList() {
  ...
  const processAggregation = useMemo(() => {
    ...
    activities.forEach((act: any) => {
      const app = act.app_name || act.appName || act.process || "Unknown App";
      const title = act.window_title || act.title || "Desktop / Idle";
      const key = `${app} - ${title}`;
      const duration = Number(act.duration_seconds || act.durationSeconds || act.duration || 0);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Realtime foreground process tracking logs pulled directly from FlowTrack backend.
          </p>
        </div>
      </div>
