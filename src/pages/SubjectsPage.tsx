...
import { StudyTemplate } from "@/data/studyTemplates";
...
export function SubjectsPage() {
  ...
  const handleSelectTemplate = (template: StudyTemplate) => {
    setName(template.name);
    setEmoji(template.icon);
    setColor(template.color === 'red' ? '#ef4444' :
             template.color === 'blue' ? '#3b82f6' :
             template.color === 'green' ? '#22c55e' :
             template.color === 'yellow' ? '#eab308' :
             template.color === 'purple' ? '#a855f7' :
             template.color === 'orange' ? '#f97316' :
             template.color === 'cyan' ? '#06b6d4' : '#6366f1');
    setWeeklyGoal(String(Math.ceil(template.recommendedGoal / 60)));
      ...
      <Panel className="space-y-6">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="flex-1 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 py-3 font-semibold text-white shadow-xl shadow-indigo-500/20 active:scale-[0.98] transition-all hover:from-indigo-600 hover:to-cyan-600"
            >
              Create New Subject
            </button>
            <button
              ...
              className="px-6 rounded-xl border border-white/20 bg-slate-800/50 py-3 font-semibold text-white hover:bg-slate-800 transition-all flex items-center gap-2"
            >
